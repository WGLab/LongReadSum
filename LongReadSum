...
def f5_module(margs):
    ...
    if not errorStr == lrst_global.originalError:
        ...
        for static in [True, False]:
            if margs.seq==0:
                f5_html_gen = generate_html.ST_HTML_Generator([["basic_st", "read_length_st","read_length_hist","base_st","basic_info"], "The statistics for F5", para_dict], static=static);
            else:
                f5_html_gen = generate_html.ST_HTML_Generator(
                   [["basic_st", "read_length_st","read_length_hist", "basic_info"], "The statistics for F5", para_dict], static=static)
