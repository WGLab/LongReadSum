...
void F5_Module::F5_do_thread(std::ifstream* ref_F5_reader_ss, Input_Para& ref_input_op, int thread_id, F5_Thread_data& ref_thread_data, Output_F5& ref_output ){
    ...
    while (true){
        ...
        for(read_ss_i=0; read_ss_i<read_ss_size; read_ss_i++){
           ...
           _f5_st->read_quality_distribution[ int( ref_thread_data._F5_ss_records[read_ss_i].mean_qscore_template ) ] += 1;
           if ( _f5_st->min_read_quality == MoneDefault || 
               _f5_st->min_read_quality>int( ref_thread_data._F5_ss_records[read_ss_i].mean_qscore_template ) ){
              _f5_st->min_read_quality = int( ref_thread_data._F5_ss_records[read_ss_i].mean_qscore_template );
           }
           if ( _f5_st->max_read_quality < int( ref_thread_data._F5_ss_records[read_ss_i].mean_qscore_template) ){
              _f5_st->max_read_quality = int( ref_thread_data._F5_ss_records[read_ss_i].mean_qscore_template); 
           }
        }
